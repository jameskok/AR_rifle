...
#объект Image является буфером в который выбрасываются обработанные изображения. Так транслируется обработанное видио
...
def change_pic(pic, lout):  #функция обновления картинки 
    if lout.ids['camera'].texture != None:  #если изображение есть
        ...
        #значениями от 0 до 255(цвет). Пока это только "колбаска". reshape приводит ее к 3х мерному тензеру размера 
